...
contract RaffleTest is CodeConstants, Test {
    ...
    function testRaffleRevertsWhenYouDontPayEnough() public {
        ...
        vm.prank(PLAYER);
        //Act
        vm.expectRevert(Raffle.Ruffle_SendMoretoEnterRaffle.selector);
        //Assert
        raffle.enterRaffle();
    }
    ...
    function testDontAllowPlayersToEnterWhileRaffleIsCalculating() public {
        ...
        vm.roll(block.number + 1); // not needed
        raffle.performUpkeep("");

        //Act
        vm.expectRevert(Raffle.Ruffle_RaffleNotOpen.selector);
        vm.prank(PLAYER);
        raffle.enterRaffle{value: enteranceFee}();
        //Asset
    }
    ...
    // CHECK UPKEEP
    function testCheckUpkeepReturnsFalseIfItHasNoBlaance () public {
        //Arrange
        vm.warp(block.timestamp + interval + 1);
    ...
    function testFulFillRandomWrodsPicksAWinnerResetsAndSendsMoney() public raffleEntered skipFork {
        // Arrange
        ...
        for (uint256 i = startingIndex; i < startingIndex + additionalEntrance; i++) {
            ...
            hoax(newPlayer, 1 ether); // address with ether from foundery
            raffle.enterRaffle{value: enteranceFee}();
        }
